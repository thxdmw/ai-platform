...
- 自定义 OpenAI 兼容模型提供方，并在服务器对话中切换模型
- 严格主机密钥校验、输出截断和命令超时
...
模型可以在缺少能力时提出一条参数化命令模板，但未经管理员在对话中确认不会保存或执行；确认后只添加
...
路径等可变值使用有类型和允许范围的参数定义，服务端校验并转义后执行，不需要为每个路径重复新增命令。
服务器对话还可配置 OpenAI 兼容提供方并切换模型，提供方 API 密钥同样加密保存。
