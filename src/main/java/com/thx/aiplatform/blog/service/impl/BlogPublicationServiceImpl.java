package com.thx.aiplatform.blog.service.impl;
...
/**
 * 发布候选生命周期实现：候选存于进程内 ConcurrentHashMap，服务重启即丢失——这符合
 * 「一次性短时确认」的语义，也免去了为一次性对象引入持久化的复杂度。
 */
@Service
public class BlogPublicationServiceImpl implements BlogPublicationService {
    ...
    @Autowired
    public BlogPublicationServiceImpl(BlogApiClient apiClient, BlogAssistantProperties properties) {
        this(apiClient, properties, Clock.systemUTC());
    }

    public BlogPublicationServiceImpl(BlogApiClient apiClient, BlogAssistantProperties properties, Clock clock) {
