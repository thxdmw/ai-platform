...
/**
 * 博客助手会话编排实现：为每次请求固定助手身份与系统提示词，并装配只读查询与发布候选
 * 两套工具。模型的行为边界（只查不编造、只生成候选不发布）主要由 SYSTEM_PROMPT 约束
 * 而非代码强制，因此提示词变更等同于行为变更，需要配套回归。
 */
@Service
public class BlogAssistantServiceImpl implements BlogAssistantService {
    ...
    public BlogAssistantServiceImpl(AssistantChatGateway chatGateway, BlogQueryTools queryTools,
