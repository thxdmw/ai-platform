...
/**
 * 服务器助手编排实现：把页面当前选中的服务器、会话与平台聊天网关组装起来，构造只属于
 * 本次对话的工具集并向模型发起流式对话。
 *
 ...
 */
@Service
public class ServerAssistantServiceImpl implements ServerAssistantService {
    ...
    public ServerAssistantServiceImpl(AssistantChatGateway chatGateway,
    ...
    public Flux<AssistantStreamEvent> stream(ServerChatRequest request) {
        ServerEntity server = configurationService.requireEnabled(request.serverId());
