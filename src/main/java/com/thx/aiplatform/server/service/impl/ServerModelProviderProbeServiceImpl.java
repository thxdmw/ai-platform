...
/**
 * 提供方连接探测实现：只读取模型目录，不发送聊天内容，既能验证地址与密钥，也不会产生模型费用。
 */
@Service
public class ServerModelProviderProbeServiceImpl implements ServerModelProviderProbeService {

    private final ServerModelProviderService modelProviderService;
    private final ServerCredentialCipher cipher;
    private final RestClient restClient;

    ServerModelProviderProbeServiceImpl(ServerModelProviderService modelProviderService, ServerCredentialCipher cipher,
    ...
    private byte[] resolveSecret(ServerModelProviderProbeRequest request) {
        ...
        ServerModelProviderEntity provider = modelProviderService.findProvider(request.providerId())
