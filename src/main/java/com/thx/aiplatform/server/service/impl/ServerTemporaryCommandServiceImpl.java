...
/**
 * ReAct 临时命令入口实现。命令不需要预先保存：服务端能证明只读时立即执行，其他情况只创建
 * 待审批动作。工作目录与命令分开校验，避免模型通过拼接 cd 和 Shell 控制符绕过风险判断。
 */
@Service
public class ServerTemporaryCommandServiceImpl implements ServerTemporaryCommandService {

    private static final Logger log = LoggerFactory.getLogger(ServerTemporaryCommandServiceImpl.class);
    ...
    public ServerTemporaryCommandServiceImpl(ServerCommandRiskClassifier riskClassifier,
