...
import com.thx.aiplatform.server.service.ServerConversationBindingService;
...
/**
 * 会话 → 服务器绑定表实现：一个对话一旦关联某台服务器就不可更换——模型会话的 system
 * prompt 与工具上下文都绑定该服务器，切换会让模型沿用上一台服务器的命令 ID 和上下文而
 * 串台。绑定是纯内存状态，用 accessOrder 的 LinkedHashMap 做 LRU 淘汰，防止长期会话把
 * 内存涨满；服务器/命令是持久化的，绑定丢了一台只是下次需重选，可以接受。
 */
@Service
public class ServerConversationBindingServiceImpl implements ServerConversationBindingService {
     ...
     */
    @Override
    public synchronized void bind(String conversationId, String serverId) {
